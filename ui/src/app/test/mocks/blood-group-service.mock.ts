...
import { BloodGroup } from '../../module/blood-group/model/blood-group';

export class MockBloodGroupService {
  gender: BloodGroup = { id: 1, value: 'BBA', status: true};
  getBloodGroups(page: number = 1, limit: number = 5) {
    // return this.http.get<Pagination<BloodGroup>>(
    //   `${this.url}?page=${page}&limit${limit}`
    // );
    const genders: Pagination<BloodGroup> = { data: [this.gender], tcount: 1 };
